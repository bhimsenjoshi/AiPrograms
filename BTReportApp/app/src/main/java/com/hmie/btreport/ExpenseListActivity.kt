...
class ExpenseListActivity : AppCompatActivity() {
        ...
        val adapter = ExpenseAdapter(
            ...
            onDelete = { exp ->
                AlertDialog.Builder(this)
                    ...
                    .setMessage("Delete this ${exp.type.displayName} expense of Rs.${"%.2f".format(exp.amount)}?")
    ...
    private fun showShareDialog(xlsx: File, docx: File) {
        AlertDialog.Builder(this)
            ...
            .setMessage("Files saved to:\n${xlsx.name}\n${docx.name}\n\nWhat would you like to do?")
            .setPositiveButton("Share Both") { _, _ -> shareFiles(listOf(xlsx, docx)) }
            .setNeutralButton("Share Excel") { _, _ -> shareFiles(listOf(xlsx)) }
            .setNegativeButton("Share Word") { _, _ -> shareFiles(listOf(docx)) }
            .show()
    }
